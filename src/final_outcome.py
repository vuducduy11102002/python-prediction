...
class ModelPredictor:
    def __init__(self, config_file_path):
        ...
        self.model = joblib.load("models/all_models.pkl")
        ...
        with tracer.start_as_current_span("processors") as processors:
            with tracer.start_as_current_span(
                ...
                if not hasattr(data, 'data'):
                    logger.error("Data object does not have 'data' attribute.")
                    return {"error": "Data object does not have 'data' attribute."}
            ...
            with tracer.start_as_current_span(
                "ensemble predict", links=[trace.Link(processors.get_span_context())]
            ):
                predictions = {}
                for i, model in enumerate(self.model):
                    with tracer.start_as_current_span(
                        f"predict_{i}", links=[trace.Link(processors.get_span_context())]
                    ):
                        model_name = self.config["models"][i]["model_name"]
                        model_predictions = model.predict(df)
                        predictions[model_name] = model_predictions.tolist()
                end_pred = time.time()
                logger.info(f"ensemble predict time: {end_pred - end_scaler}")

            # Majority voting to choose the final prediction
            final_predictions = [1 if sum(p) > len(p) / 2 else 0 for p in zip(*predictions.values())]

            return {"id": data.id, "predictions": predictions, "out_come": final_predictions}
